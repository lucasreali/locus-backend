import { syllabusQueue } from '@/modules/syllabus/syllabus.queue';
import { syllabusRepository } from '@/modules/syllabus/syllabus.repository';
import { syllabusService } from '@/modules/syllabus/syllabus.service';
import { ForbiddenError } from '@/shared/errors/ForbiddenError';
import { NotFoundError } from '@/shared/errors/NotFoundError';
import { beforeEach, describe, expect, it, vi } from 'vitest';

vi.mock('@/modules/syllabus/syllabus.repository', () => ({
	syllabusRepository: {
		create: vi.fn(),
		findById: vi.fn(),
...
vi.mock('@/modules/syllabus/syllabus.queue', () => ({
	syllabusQueue: {
		add: vi.fn(),
	},
}));
...
type SyllabusCreateResult = Awaited<ReturnType<typeof syllabusRepository.create>>;
type SyllabusFindByIdResult = Awaited<
	ReturnType<typeof syllabusRepository.findById>
>;
type SyllabusFindAllByUserIdResult = Awaited<
	ReturnType<typeof syllabusRepository.findAllByUserId>
>;

type MockSyllabusRecord = NonNullable<SyllabusFindByIdResult>;

const mockSyllabusRecord: MockSyllabusRecord = {
	id: SYLLABUS_ID,
	userId: USER_ID,
	fileName: 'test.pdf',
	documentType: null,
...
describe('syllabusService.upload', () => {
	it('should create a DB record with PENDING status, enqueue the job, and return the created record', async () => {
		const mockCreated = {
			...mockSyllabusRecord,
			status: 'PENDING',
		} as SyllabusCreateResult;
		vi.mocked(syllabusRepository.create).mockResolvedValue(mockCreated);
		vi.mocked(syllabusQueue.add).mockResolvedValue({} as never);
...
describe('syllabusService.findById', () => {
	it('should return the record when found and userId matches', async () => {
		vi.mocked(syllabusRepository.findById).mockResolvedValue(mockSyllabusRecord);

		const result = await syllabusService.findById(USER_ID, SYLLABUS_ID);

		expect(result).toEqual(mockSyllabusRecord);
	...
	it('should throw NotFoundError when record does not exist', async () => {
		vi.mocked(syllabusRepository.findById).mockResolvedValue(
			undefined as unknown as SyllabusFindByIdResult,
		);

		await expect(syllabusService.findById(USER_ID, SYLLABUS_ID)).rejects.toThrow(
			NotFoundError,
		);
		await expect(syllabusService.findById(USER_ID, SYLLABUS_ID)).rejects.toThrow(
			'Syllabus record not found',
		);
	});
...
describe('syllabusService.findAllByUserId', () => {
	it('should return all records for the user', async () => {
		vi.mocked(syllabusRepository.findAllByUserId).mockResolvedValue([
			mockSyllabusRecord,
		] as SyllabusFindAllByUserIdResult);
...
describe('syllabusService.deleteById', () => {
	it('should delete the record when found and userId matches', async () => {
		vi.mocked(syllabusRepository.findById).mockResolvedValue(mockSyllabusRecord);
