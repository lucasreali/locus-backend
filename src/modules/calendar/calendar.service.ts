...
import type { calendarEventRequestStatic, calendarEventUpdateRequestStatic } from "./calendar.dto";
...
export const calendarService = {
	...
	async findAllByUserId(userId: string, startDate?: string, endDate?: string) {
        if (startDate && endDate) {
            return await calendarRepository.findByUserIdAndDateRange(userId, new Date(startDate), new Date(endDate));
        }
		return await calendarRepository.findByUserId(userId);
	},

	async updateById(userId: string, eventId: string, data: calendarEventUpdateRequestStatic) {
		const [event] = await calendarRepository.findById(eventId);
